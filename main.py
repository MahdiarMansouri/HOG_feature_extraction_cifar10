...
for idx, img in enumerate(X_train):
    ...
    img_combined = np.dstack((img, img_hog))
...
model.add(Conv2D(64, input_shape=(32, 32, 6), kernel_size=(3, 3), padding='same', activation='relu'))
...
# print(np.array(X_train_combined).shape)
# Train model
history = model.fit(
    np.array(X_train_combined),
    y_train,
    batch_size=64,
...
# Evaluate model
loss, accuracy = model.evaluate(np.array(X_test_combined), y_test)
print(f'The Loss is {loss} and Accuracy is {accuracy}')
